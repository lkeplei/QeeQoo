...
void CCBAnimationManager::runAnimations(int nSeqId, float fTweenDuration)
{
	...
	//解决和CCAction结合的时候出现action运行时间很短的问题(mRootNode->stopAllActions();)
	
    mRootNode->stopActionByTag(KRootActionTag);
	...
	completeAction->setTag(KRootActionTag);
	...
	//计时
	CCDirector::sharedDirector()->getScheduler()->scheduleUpdateForTarget(this, kCCPrioritySystem, false);
}
...
void CCBAnimationManager::stopAnimation()
{
	CCDirector::sharedDirector()->getScheduler()->unscheduleUpdateForTarget(this);
	mRootNode->stopActionByTag(KRootActionTag);
	//mRootNode->stopAllActions();
...
void CCBAnimationManager::update(float dt){
